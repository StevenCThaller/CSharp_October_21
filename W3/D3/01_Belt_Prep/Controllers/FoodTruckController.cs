...
namespace _01_Belt_Prep.Controllers
{
    public class FoodTruckController : Controller 
    {
        ...
        public FoodTruckController(MyContext context)
        {
            _context = context;
        }
        ...
        public IActionResult CreateReview(int truckId, Review fromForm)
        {
            ...
            if(ModelState.IsValid)
            {
                // Check if the logged in user has already reviewed this truck.
                FoodTruck toReview = _context.FoodTrucks.Include(t => t.Reviews).FirstOrDefault(t => t.FoodTruckId == truckId);

                if(toReview.Reviews.Any(r => r.UserId == (int)LoggedInUserId))
                {
